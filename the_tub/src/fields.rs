...
impl VectorField2D {
    ...
    fn find_neighbor_axis_lines(&self, axis_index: usize, value: f64) -> [AxisLine; 2] {
        ...
        let index = self.axes[axis_index]
            .as_slice()
            .unwrap()
            .partition_point(|&x| x < value);
        [
            index.checked_sub(1).unwrap_or(axis.len() - 1),
            index % self.axes[axis_index].len(),
        ]
        .map(|index| AxisLine {
            index,
            value: self.axes[axis_index][index],
        })
    }
    ...
    fn interpolate(&self, point: nd::Array1<f64>) -> nd::Array1<f64> {
        ...
        let [x_axis_lines, y_axis_lines] = [
            self.find_neighbor_axis_lines(0, point[0]),
            self.find_neighbor_axis_lines(1, point[1]),
        ];
        ...
        for x_line in x_axis_lines.as_slice() {
            for y_line in y_axis_lines.as_slice() {
                ...
                let wt =
                    ((point[0] - x_line.value) * (point[1] - y_line.value) / denominator).abs();
